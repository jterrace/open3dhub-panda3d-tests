...
def get_pixel_errors(files, basepath):
    ...
    for i, file in enumerate(files):
        ...
        p = subprocess.Popen([PERCEPTUAL_DIFF, '-threshold', '1', cur_path, blessed_path], stdout=subprocess.PIPE)
        p.wait()
        ...
        error_vals.append((timeval, error_pixels))
        
        print '%s - %d out of %d' % (basepath, i+1, len(files))
